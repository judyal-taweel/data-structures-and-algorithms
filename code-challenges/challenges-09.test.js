...
function transformToLis(obj){
  // Solution code here...
  let newObj = Object.entries(obj).map(val => `<li>${val}</li>`)
  return newObj;
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  return arr.reduce((value) => value += 1, 0);
...
const returnNames = (arr) => {
  // Solution code here...
  return Object.values(arr.reduce((value, obj, index) => {
    value[index] = obj.name;
    return value;
  }, {}));
...
const reversedString = (str) => {
  // Solution code here...
  let reverse=[str];
  let revStr = reverse.reduce((a,value,i)=>{
   return a+=value[value.length-i];
  },[])
  return revStr;
